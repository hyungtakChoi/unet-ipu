...
def double_conv_layer(filter_size, x, dtype, name):
    x = Conv2D(filter_size, 3, strides=1, padding="same", activation="relu", kernel_initializer="he_normal", dtype=dtype, name=name + "_conv_0")(x)
    x = Conv2D(filter_size, 3, strides=1, padding="same", activation="relu", kernel_initializer="he_normal", dtype=dtype, name=name + "_conv_1")(x)
    return x
