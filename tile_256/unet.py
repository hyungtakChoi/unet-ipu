...
def get_optimizer(args):
    ...
    if args.optimizer == "adam":
        ...
        learning_rate_fn = tf.keras.optimizers.schedules.CosineDecay(
            args.learning_rate, decay_steps=args.num_epochs
        )
        optimizer_instance = keras.optimizers.Adam(
            learning_rate=learning_rate_fn, epsilon=1e-4, gradient_transformers=[gradient_normalizer]
        )
    else:
...
def train_model(args, model, ds_train, ds_eval):
    ...
    t1 = perf_counter()
    duration = t1 - t0
    logger.info(f"Inference\t Time: {duration} seconds\t")
    if args.eval:
